...
def HPF(hash)
    @rp_table = Hash.new #hash(アドレス|RP1)
    ...
    if @a <= 8
        for i in 0..@a-1
            @rp_table.store(@hash_high_key[i],4 - 0.5 * i)
        end
    else
        for i in 0..@a-1
            @rp_table.store(@hash_high_key[i],(4 - (4 / @a.to_f) * i).to_d.floor(1).to_f)
        end
    end

    # 中央値以上のアイテムはRP=0
    for i in 0..@hash_low.size-1
        @rp_table.store(@hash_low_key[i],0)
    end
...
def RP
    ...
    @hash_key = @rp1_table.keys # ハッシュのキー（配列）
    # 未完成
    rp_table = [@rp1_table,@rp2_table]
    for i in 0..@hash_key .size-1
        rp_table.inject(0) {|sum, hash| sum + hash[@hash_key[i]]} 
    end
    p rp_table
end
